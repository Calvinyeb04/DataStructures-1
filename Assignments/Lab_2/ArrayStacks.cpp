...
class arrayStack{
    ...
    public:
        arrayStack(int intitalCapacity){
            ...
            array = new int[orignalCapacity]; // - Tell the Array to Create the Space for the Origanl Cap
            top = -1;
        }
        ...
        void push(int val){
            if(top == orignalCapacity -1){
                int newCapacity = orignalCapacity * 2 ;// - Create A New Capacity Then Multiply by 2
        ...
        void pop(){
            //is the Stack Empty;  //- To Tracker(Top) has to point to a spot thats empty. // ARRAY SLOTs ARE NUmBERED starting from 0
            if(top == -1){
